...
from f1tenth_augmentation.utils import create_sysdata_from_file, create_rnd_train_and_val_data_from_file, plot_losses, create_orth_data_from_file
...
experiment = "sim"  # "sim" or "meas" for simulation or measurement data based identification
cwd = os.path.dirname(__file__)
save_name = f'../results/{experiment}/additive_static_augment_' + (datetime.now()).strftime("%Y%m%d-T%H%M%S")
...
fit_sys = get_augmented_fitsys(augmentation_structure=SSE_AdditiveAugmentation, known_system=bb_model, neur_net=ffw_net,
                               e_net=None, regLambda=regLambda, orthLambda=orthLambda, norm_data=train, norm_x0_meas=True)
fit_sys.init_model(test, auto_fit_norm=False)

if orthLambda != 0:
    orth_data = create_orth_data_from_file(train_data_folder)
    U_orth, X, U = calculate_orthogonalisation(sys=bb_model, train_data=orth_data, x_meas=True)
...
fit_sys.fit(train_sys_data=train, epochs=epoch, val_sys_data=valid, batch_size=batch_size, loss_kwargs=loss_kwargs,
...
if bb_model.parm_corr_enab:
    ...
    parm_corr_data = {
        ...
        "beta (orthogonalization)": orthLambda,
        "m": P[0],
        "Jz": P[1],
        "lr": P[2],
        "lf": P[3],
