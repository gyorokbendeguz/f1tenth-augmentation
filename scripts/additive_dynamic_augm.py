...
from model_augmentation.augmentation_structures import get_dynamic_augment_fitsys, SSE_AdditiveDynAugmentation


experiment = "sim"  # "sim" or "meas" for simulation or measurement data based identification
cwd = os.path.dirname(__file__)
save_name = f'../results/{experiment}/additive_dynamic_augment_' + (datetime.now()).strftime("%Y%m%d-T%H%M%S")
...
n_in = nx + nu + nx_hidden  # states + inputs
...
bb_model = nonlinearCar(nx=nx, ny=ny, nu=nu, ts=Ts, parmTune=bParmTune)

# Initialization
fit_sys = get_dynamic_augment_fitsys(augmentation_structure=SSE_AdditiveDynAugmentation, known_system=bb_model,
