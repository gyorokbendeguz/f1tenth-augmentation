...
save_name = f'../results/{experiment}/LFR_dynamic_augment_' + (datetime.now()).strftime("%Y%m%d-T%H%M%S")
...
bb_model = nonlinearCar(nx=3, ny=3, nu=2, ts=Ts, parmTune=bParmTune)

# Initialization
fit_sys = get_dynamic_augment_fitsys(augmentation_structure=SSE_LFRDynAugmentation, known_system=bb_model,
                                     neur_net=ffw_net, e_net=None, y_lag_encoder=lag_all, u_lag_encoder=lag_all,
                                     enet_kwargs=dict(n_nodes_per_layer=n_nodes_per_layer_enc,
                                                      n_hidden_layers=n_hidden_layers_enc, activation=n_act),
                                     regLambda=regLambda, norm_data=train, norm_x0_meas=True, hidden_state=nx_hidden)
